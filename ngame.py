...
def Bulbasaur():
    ...
    ant = input ("Wil je naar Pewter City gaan? ").lower()
    if ant == ("ja"):
        succesBulbasaur1()
    if ant == ("nee"):
        print ("Dan niet, blijf maar lekker chillen thuis, dan word mijn kleinzoon GARY easy kampioen")

def succesBulbasaur1():
    print ("Helemaal prima! Succes met je avontuur!")
    ant = input ("Terwijl je onderweg was naar Pewter City is je Bulbasaur level 9 geworden! Nu ga je je de gym verslaan! Nu je bij de gym bent aangekomen en tegen Brock, de steen pokemon gym leader vecht mag je een attack kiezen! Welke attack wil je doen:  VineWhip of FlameThrower? ").lower()
    if ant == ("vinewhip"):
        succesBulbasaur2()
...
def succesBulbasaur2():
    ...
    ant = input ("Nu je naar de volgende stad bent gereisd is Ivysaur een paar levels omhoog gegaan, het is nu level 21! Goed bezig!      Alleen nu je bent aangekomen in Cerulean City moet je tegen Misyt, de water gym leader vechten. Ook al is Ivysaur qua   type in het voordeel kan het niet zwemmen. Geven we Ivysaur zwembandjes? Beantwoord met ja of nee. ").lower()
    if ant == ("ja"):
        succesBulbasaur3()
    elif ant ==("nee"):
        print ("Helaas in Ivysaur verdronken dankzij jouw slechte keuze. GAME OVER")

def succesBulbasaur3():
...
def succesBulbasaur4():
    ...
    print("Tip: probeer een andere starter pokemon te kiezen")
...
def succesSquirtle3():
    ...
    ant = input ("Ben je er klaar voor om Pokemon meester te worden? Beantwoord met ja of nee. ").lower()
    if ant == ("ja"):
        ...
        print ("Haha wat een domme fout, je hebt een hele sterke pokemon en nu je nee hebt gezegt word mijn kleinzoon kampioen. GAME OVER")

def Charmander():
    print ("Charmander ligt te slapen en heeft geen zin om wakker gemaakt te worden.")
    ant = input ("Maak je Charmander alsnog wakker? Beantwoord de vraag met ja of nee. ").lower()
    if ant == ("ja"):
